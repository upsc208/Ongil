...
import ReportPostApi from "../../../api/blog-services/report/ReportCommentApi";
...
const PostReportPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log("Request Payload:", { postId, reason: convertedReason, details });
      const {status} = await ReportPostApi(postId, convertedReason, details);
      if (status === 200) {
        alert("신고가 접수되었습니다.");
      }
    } catch (error) {
